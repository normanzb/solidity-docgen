...
export async function compile(
  ...
  if (allErrors && allErrors.some((e) => e.severity === "error")) {
    ...
    const moreErrors =
      errors.length === 1 ? "" : ` (And ${errors.length - 1} other errors...)`;
    throw new Error(
      `Solidity was unable to compile. ${firstError}${moreErrors}`
    );
  }
    ...
    const adaptDocumentation = (node: any) => {
      if (typeof node.documentation === "string") {
        ...
        if (source !== undefined) {
          ...
          node.documentation = cleanUpDocstringFromSolc(
...
class ASTReader {
  ...
  private decodeSrc(src: string): {
    source: string;
    start: number;
    length: number;
  } {
    const [start, length, sourceId] = src.split(":").map((s) => parseInt(s));
    const source = Object.keys(this.output.sources).find(
      (s) => this.output.sources[s].id === sourceId
    );
    if (source == null) {
